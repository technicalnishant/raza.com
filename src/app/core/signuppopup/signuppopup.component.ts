...
export class SignuppopupComponent extends AppBaseComponent implements OnInit {
  ...
  private registerUsingPhoneNumber(phoneNumber: string, country: Country) {
    this.executeCaptcha('login').toPromise().then(token => {
      ...
      this.authService.sendOtpForRegister(phoneNumber, token).toPromise()
        .then(res => {
          // Success: Handle successful OTP sending here
          this.showPinBox = true;
          this.processOtp = true;
        })
        .catch(error => {
          // Error: Handle error message here
          if (error.Message === "Invalid phone number. Please check your phone number and try again.") {
            console.error("Error: Invalid phone number.");
            // Show an error message to the user (using your preferred method, e.g., a dialog or notification)
            //this.showErrorMessage("Invalid phone number. Please check your phone number and try again.");
            this.signUpForm.controls['phoneOrEmail'].setErrors({ 'invalid_input': true });
          } else {
