...
export class GlobalcallComponent implements OnInit {
  ...
  countryFrom: Country[];
  currentCurrency:any;
  logedInRatesList:any;
  logedInRatesListTop:any;
  logedDenomination : any;
  loginPhone:any;
  constructor(
...
getTotalMin1(data, price)
{

 
  let  item  = data.Denominations.filter(a => a.Price == price);
 
  return item[0].TotalTime + item[0].PromoMinutes;
 ...
 getFlagName()
 {
  ...
  getRates()
  {
    ...
    {
      this.searchRatesService.getMySearchGlobalRates(this.currentSetting.currentCountryId, this.countryId, this.loginPhone).subscribe(
        (data: any) => {

          this.logedInRatesList     = data;
          ...
          this.SubPlans             = sortedList;
          let filterData            = data.filter(a=>a.CountryId == this.countryId );
          let plan_info             = filterData[0].Denominations;

          plan_info = plan_info.sort((a, b) => {
            const countryNameA = a.Price;
            const countryNameB = b.Price;
          ...
          this.logedDenomination = plan_info;
          this.logedInRatesListTop  = filterData[0];

          this.denominationList     = Array.from(new Set(this.logedDenomination
            .map(item => item.Price)));
        ...
        // this.denominationList      = this.denominationList.filter(a=> a != 90 )
         ...
         this.baseCountryId = this.logedInRatesListTop.CountryId;
         this.baseCountryName = this.logedInRatesListTop.CountryName;
 
         this.countryId = this.logedInRatesListTop.CountryId;
         this.countryName = this.logedInRatesListTop.CountryName;
         this.selectedPrice = this.denominationList[1];
         ...
         this.filterDetailRate(this.denominationList[0]);
          ...
          console.log('this.logedInRatesListTop', this.logedInRatesListTop);
  ...
  filterDetailRate(price: number) 
  {
    ...
    console.log('this.FilteredSubPlans', this.FilteredSubPlans);
