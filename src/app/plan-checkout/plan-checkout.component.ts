...
@Component({
  ...
  styleUrls: ['./plan-checkout.component.scss']
})
export class PlanCheckoutComponent implements OnInit {
  ...
  route_autorefill : boolean = false;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      const subcardid = params['subcardid'];
      ...
      const countryFrom = params['countryfrom'];
      const isAutorefill = params['autorefill'];
      const countryTo = params['countryto'];
      const price = params['price'];
  
      this.countryFrom = countryFrom;
      this.selectedPrice = price;
      this.countryId = countryTo;
      this.isAutorefill = isAutorefill;
...
getTotalMin1(item, price)
{
 ...
 getFlagName()
 {
  ...
  filterPriceAndCheckout()
  {
    ...
    let plan = (this.isAutoRefill)?this.AutorefillPlans:this.WithoutAutorefillPlans;

    let item = plan.filter(a => a.Price == this.selectedPrice);
   // console.log("Selected Price item",item[0])
    this.onClickRateTab(item[0]);
  }
  contactUs() {
